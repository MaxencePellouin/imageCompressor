...
instance Show ImcInfos where
    show (ImcInfos n l f) = show f ++ " with " ++ show n ++ " klusters and "
        ++ show l ++ " convergence"
...
parseArgs [] i = Just i
parseArgs ("-n":x:xs) (ImcInfos n l f) = parseArgs xs
    (ImcInfos (read x :: Int) l f)
parseArgs ("-l":x:xs) (ImcInfos n l f) = parseArgs xs
    (ImcInfos n (read x :: Float) f)
parseArgs ("-f":x:xs) (ImcInfos n l f) = parseArgs xs
    (ImcInfos n l x)
